...
<Warning>
  Before v0.5 model name was used in chat history key by default:
  `AgentName_ModelName_UserId`. To keep old code compatible set it to `true` in
  your agent classes.
</Warning>
...
### Random Chat Histories

If you don't specify chat history name with `for` or `forUser` methods, there will be used a random chat key:

```php Without history name
// Get instance with make and keep it chainable
$response = MyAgent::make()->respond("How do I make pasta carbonara?");

// Directly ask to the agent
$response = MyAgent::ask('Will it rain tomorrow?');

```

By default, there is set 10 random characters with `Str` facade, but you can customize the automatic key generation by overriding the `generateRandomKey` static method in your Agent class:

```php
// Auto chat history with user type and id
protected static function generateRandomKey(): string {
  $user = auth()->user();
  return $user->type . "-" . $user->id;
}

// Auto chat history with agent class name and topic
protected static function generateRandomKey(): string {
...
<Note>
  You can use `generateRandomKey` method to set non-random, but dynamic or
...
<Warning>
  Before v0.5 keys was stored as `promptTokens` (camelCase) instead of
  `prompt_tokens` (snake_case), so make sure to update your code if you were
  using it.
</Warning>
